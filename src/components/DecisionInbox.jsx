...
export default function DecisionInbox({ decisions, onRespond }) {
  ...
  const handleRespond = (decision) => {
    if (!response.trim() || decision.isResolving) return
            ...
            <div key={decision.id} className="border border-warn/30 bg-warn/5 rounded p-3 mb-2">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {decision.resolveError && (
                    <p className="text-xs text-warn mt-1">{decision.resolveError}</p>
                  )}
                </div>
              </div>
              ...
              {selected === decision.id ? (
                <div className="mt-3">
                  <textarea
                    ...
                    className="w-full bg-void border border-border rounded px-3 py-2 text-sm text-text placeholder-muted outline-none focus:border-accent/50 transition-colors font-mono resize-none disabled:opacity-60"
                  />
                  <div className="flex gap-2 mt-2">
                    <button
                      onClick={() => handleRespond(decision)}
                      disabled={decision.isResolving || !response.trim()}
                      className="text-xs bg-accent text-void font-bold px-3 py-1.5 rounded hover:opacity-90 transition-opacity disabled:opacity-50"
                    >
                      {decision.isResolving ? 'SENDING...' : 'SEND DECISION'}
                    </button>
                    <button
                      onClick={() => setSelected(null)}
                      disabled={decision.isResolving}
                      className="text-xs text-muted border border-border px-3 py-1.5 rounded hover:border-muted transition-colors disabled:opacity-50"
                    >
                      CANCEL
                    </button>
                  </div>
                ...
                <button
                  ...
                  disabled={decision.isResolving}
                  className="mt-2 flex items-center gap-1 text-xs text-warn hover:text-warn/80 transition-colors disabled:opacity-50"
                >
                  {decision.isResolving ? 'SENDING' : 'RESPOND'} <ChevronRight size={12} />
                </button>
              )}
            </div>
          ))}
